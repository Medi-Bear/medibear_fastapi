...
import os, math, base64, tempfile
import numpy as np
import cv2
...
def angle_3pts(a, b, c):
    ...
    cosv = np.clip((ba[0]*bc[0] + ba[1]*bc[1]) / denom, -1.0, 1.0)
    return round(math.degrees(math.acos(cosv)), 1)


def extract_pose_features(bgr_img):
    h, w = bgr_img.shape[:2]
    ...
    back_angle  = angle_3pts(SHO, HIP, KNE)

    if (knee_left+knee_right)/2 < 100 or (elbow_left+elbow_right)/2 < 100:
        stage = "down"
...
def on_startup():
    ...
    "../../models/exercise_models/cnn_lstm_model.keras",
    custom_objects={"TemporalAttention": TemporalAttention},
    compile=False,
    safe_mode=False   # ✅ 핵심 옵션
    )


    pose_detector = mp_pose.Pose(static_image_mode=False)  # ✅ 실시간 최적화

    print("✅ 모델 로드 완료, classes =", CLASSES)
...
# import os, io, math, base64, tempfile
# import numpy as np
# import cv2
...
# # --------------------------
# # ✅ (수정됨) Video 처리: OpenCV 기반
# # --------------------------
# def predict_exercise_from_video_bytes(video_bytes):
...
#                 "exercise_confidence": ex["exercise_confidence"],
#                 "probs": ex["probs"],
#                 "stage": pose["stage"],
...
#     last = frames_data[-1] if frames_data else None
#     return {
#         "detected_exercise": overall["detected_exercise"],
#         "exercise_confidence": overall["exercise_confidence"],
#         "probs": overall["probs"],
#         "frames": frames_data,
#         "total_frames": len(frames_data),
#         "stage": last["stage"] if last else None,
#         "pose_detected": last["pose_detected"] if last else None,
#         "pose_data": last["pose_data"] if last else None
#     }
...
# # --------------------------
# import requests
# LLM_SERVER_URL = "http://127.0.0.1:8000/chat_with_analysis"

# @app.post("/analyze_and_chat")
# def analyze_and_chat(payload: AnalyzeRequest):
#     try:
#         if payload.image:
...
#         else:
#             analysis = {"message": payload.message}
...
#         res = requests.post(LLM_SERVER_URL, json=llm_payload)
